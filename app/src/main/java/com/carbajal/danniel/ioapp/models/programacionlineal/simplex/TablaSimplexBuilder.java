	...
	private static void renglonesRestricciones(ArrayList<Restriccion> restricciones, double[][] tablaSimplex){
		...
		for (int i=0;i<numeroRestricciones;i++){
			...
			for (int j=0;j<variablesRestriccion.size();j++){
				...
				if (i==j){
					tablaSimplex[1+i][j+variablesRestriccion.size()] = (
						restriccion.getIgualdad()== TiposIgualdades.IGUAL
						|| restriccion.getIgualdad()== TiposIgualdades.MENOR
						|| restriccion.getIgualdad()== TiposIgualdades.MENOR_IGUAL)?1:-1;
				}
			}
